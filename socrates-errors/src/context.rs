...
impl<'input> ErrorContext<'input> {
    ...
    pub fn block_exec<U, F: FnOnce(&mut Self) -> Result<U, failure::Error>>(&mut self, f: F) -> Result<U, failure::Error> {
        ...
        issues.sort_by_key(|(_, e)| e.location().unwrap_or((0, 0)));
         ...
         for (is_error, issue) in &issues {
            writeln!(
                out,
                "{}{}{}",
                if *is_error { "error".bright_red() } else { "warning".bright_yellow() }.bold(),
                ": ".bold(),
                format!("{}", issue).bold())?;
            ...
            if let Some((start, end)) = issue.location() {
                ...
                let end_col = if end_line != line { line_source.len() } else { end_col };
                let col_1 = format!("{} ", line + 1);

                let pipe = "|".bright_blue().bold();

                writeln!(out, "{0: <1$}{2} {3}:{4}:{5}", "", col_1.len() - 1, "-->".bright_blue().bold(), self.filename, line + 1, start_col + 1)?;
                writeln!(out, "{0: <1$}{2}", "", col_1.len(), pipe)?;
                writeln!(out, "{}{} {}", col_1.bright_blue().bold(), pipe, line_source)?;
                ...
                writeln!(
                    ...
                    pipe,
                    start_col,
                    if *is_error { line.bright_red() } else { line.bright_yellow() })?;

                if let Some(note) = issue.note() {
                    writeln!(out, "{0: <1$}{2} {3} {4}", "", col_1.len(), "=".bright_blue().bold(), "note:".bold(), note)?;
                }
            }
            else {
    ...
    fn get_line(&self, line: usize) -> &str {
        ...
        let next_line_start = if line == line_count - 1 { self.source.len() } else { self.line_starts[line + 1] - 1 };
