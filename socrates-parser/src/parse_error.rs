...
#[derive(Debug)]
pub struct WrappedLalrpopError<'input>(
    pub ParseError<usize, parser::Token<'input>, &'static str>
);
...
impl<'input> error::Error for WrappedLalrpopError<'input> {
    ...
    fn cause(&self) -> Option<&error::Error> { self.0.cause() }
}
...
impl<'input> CompilerError for WrappedLalrpopError<'input> {
    fn location(&self) -> Option<(usize, usize)> {
        match &self.0 {
            ...
            ParseError::UnrecognizedToken { token: Some((start, _, end)), .. } => Some((*start, *end)),
            ParseError::UnrecognizedToken { token: None, .. } => None,
            ParseError::ExtraToken { token: (start, _, end) } => Some((*start, *end)),
