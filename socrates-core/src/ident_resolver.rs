...
use socrates_ast::parsed::{ActiveType, BinaryRelationOperator, Formula, IdentifierType, Term};
    ...
    match f {
        Predicate(name, args) => {
            if let Some(predicate) = types.get_predicate(&name.inner) {
                ...
                errors.push_error_message(
                    name.pos,
                    format!("predicate `{}` does not exist", name.inner),
                );
            }
        ...
        BinaryRelation(op, ref mut lhs, ref mut rhs) => {
            let term_context = match &op.inner {
                BinaryRelationOperator::Equality | BinaryRelationOperator::Inequality => {
                    TermContext::Unknown
                }
            ...
            for v in &v.inner {
                if let Some(type_ref) = types.get_instance(&v.inner.0) {
                    errors.push_warning_message(
                        v.pos,
                        format!(
                            "variable `{}` shadows instance of type `{}`",
                            v.inner.0,
                            types.type_name(type_ref)
                        ),
                    );
                }
            }
...
fn assign_atoms_term<'i>(
    t: &mut Spanning<Term<'i>>,
    types: &TypeStorage<'i>,
    ...
    match &mut t.inner {
        ...
        Function(name, args) => {
            if let Some(function) = types.get_function(&name.inner) {
                ...
                errors.push_error_message(
                    name.pos,
                    format!("function `{}` does not exist", name.inner),
                )
            }
            for mut arg in &mut args.inner {
                assign_atoms_term(&mut arg, types, scope, errors, TermContext::Boolean);
