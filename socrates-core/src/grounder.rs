...
use gaf::{GAFStorage, GAF, GroundTerm};
use socrates_ast::simple::{Formula, Term};
use socrates_ast::parsed::{Quantifier, BinaryRelationOperator, IdentifierType};
use cnf_receiver::CNFReceiver;
use cnf::CNFFormula;

pub trait Emitter<'i> {
    type FormulaResult;
    type ProblemResult;
    fn emit(&mut self, formula: &Formula<'i>, types: &TypeStorage<'i>)
...
impl<'i, R: CNFReceiver> ToplevelCNFEmitter<'i, R> {
    ...
    fn emit_universal_quantifier(
        ...
        for assignment in variable_assignment_iter(&variables, types) {
            ...
            self.with_pushed_scope(
                new_scope,
                |emitter| emitter.emit(formula, types))?;
        }
        Ok(())
    }
    ...
    fn emit_buffered(&mut self, formula: &Formula<'i>, types: &TypeStorage<'i>) -> Result<(), Error> {
...
impl<'i, R: CNFReceiver> Emitter<'i> for ToplevelCNFEmitter<'i, R> {
    ...
    fn emit(&mut self, formula: &Formula<'i>, types: &TypeStorage<'i>) -> Result<(), Error> {
        ...
        match formula {
            ...
            s @ Quantified(Quantifier::Existential, _, _, _) =>
                self.emit_buffered(s, types),
            Conjunction(lhs, rhs) => {
                self.emit(lhs, types)?;
                self.emit(rhs, types)?;
                Ok(())
            ...
            s @ Disjunction(_, _) =>
                self.emit_buffered(s, types),
            Predicate(sign, name, args) =>
                self.emit_predicate(*sign, name, args, types),
            s @ Constraint(_, _, _) =>
                self.emit_buffered(s, types),
...
impl<'i> BufferingCNFEmitter<'i> {
    fn emit(scope: Option<Rc<Scope<'i>>>, formula: &Formula<'i>, gaf_storage: GAFStorage<'i>, types: &TypeStorage<'i>) -> Result<CNFFormula, Error> {
...
impl<'i> Emitter<'i> for BufferingCNFEmitter<'i> {
    ...
    fn emit(&mut self, formula: &Formula<'i>, types: &TypeStorage<'i>) -> Result<CNFFormula, Error> {
        use socrates_ast::simple::Formula::*;
        match formula {
            Quantified(Quantifier::Universal, s, vs, f) =>
            ...
            Predicate(sign, name, args) =>
                self.construct_predicate(*sign, name, args, types),
            Constraint(op, lhs, rhs) =>
                self.construct_constraint(*op, lhs, rhs, types),
...
fn ground_term<'i>(term: &Term<'i>, gafs: &GAFStorage<'i>, scope: &Option<Rc<Scope<'i>>>) -> GroundTerm<'i> {
    match term {
        ...
        Term::Identifier(_, IdentifierType::Variable(s, v)) =>
            scope.as_ref().expect("ICE: No scope when encountering variable")
                .lookup(*s, *v).clone(),
...
) -> Option<bool>
{
    ...
    match (lhs, rhs) {
        ...
        (GroundTerm::Named(lhs_name, lhs_args), GroundTerm::Named(rhs_name, rhs_args)) => match op {
...
impl<'i> Scope<'i> {
    ...
    fn make_subscope(parent: &Option<Rc<Scope<'i>>>, index: u32, variable_values: Vec<GroundTerm<'i>>) -> Rc<Self> {
        Rc::new(Scope {
            index, variable_values, parent: parent.clone(),
        })
    }
}
