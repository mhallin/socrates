...
use socrates_ast::parsed::{TypeRef, ActiveType, TypeSpec};
use socrates_errors::ErrorContext;
use printer::DisplayType;

#[derive(Debug)]
enum Type<'i> {
    Uninterpreted { name: &'i str, params: Vec<TypeRef>, },
    Interpreted { name: &'i str, super_type: TypeRef, instances: Vec<&'i str>, },
    Integer { name: &'i str, min: i64, max: i64, },
}
...
#[derive(Debug)]
pub enum Function<'i> {
    Uninterpreted { name: &'i str, return_type: ActiveType, arg_types: Vec<ActiveType>, generics: Vec<(&'i str, Option<TypeRef>)>, },
    Numeric { name: &'i str, arg_types: Vec<ActiveType>, generics: Vec<(&'i str, Option<TypeRef>)>, },
}
...
impl<'i> TypeStorage<'i> {
    ...
    pub fn add_uninterpreted_type(&mut self, spec: &TypeSpec<'i>, errors: &mut ErrorContext<'i>) -> Result<(), Error> {
        let params = errors.block_exec(|errors| {
            let params = transpose_option(spec.args
                .as_ref()
                .map(|args| args.iter()
                    .map(|arg| 
                        self.get_type(&arg.inner.name)
                        .map(|t| t)
                        .ok_or_else(|| errors.push_error_message(arg.pos, format!("type `{}` not found", arg.inner.name.inner))))
                    .collect::<Vec<_>>()
                    .into_iter()
                    .collect::<Result<Vec<_>, _>>()));

            if self.get_type(&spec.name).is_some() {
                errors.push_error_message(spec.name.pos, format!("type `{}` is already defined", spec.name.inner));
            }
    ...
    pub fn add_interpreted_type(&mut self, name: &Spanning<&'i str>, super_type: &TypeSpec<'i>, instances: &[Spanning<&'i str>], errors: &mut ErrorContext<'i>) -> Result<(), Error> {
        let (super_type, instances) = errors.block_exec(|errors| {
            let super_type = match self.type_name_lookup.get(&super_type.name.inner) {
                ...
                None => {
                    errors.push_error_message(super_type.name.pos, format!("type {} not found", super_type.name.inner));
                    None
                }
            };
            ...
            let instances = instances.iter()
                .map(|i| self.instances.get(&i.inner)
                    .map(|_| {
                        errors.push_error_message(i.pos, format!("instance {} already defined", i.inner));
                        Err(())
                    })
        ...
        self.subtypes.entry(super_type)
    ...
    pub fn add_integer_type(&mut self, name: &Spanning<&'i str>, min: &Spanning<i64>, max: &Spanning<i64>, errors: &mut ErrorContext<'i>) -> Result<(), Error> {
    ...
    pub fn add_predicate(&mut self, name: &Spanning<&'i str>, args: &Spanning<Vec<Spanning<TypeSpec<'i>>>>, errors: &mut ErrorContext<'i>) -> Result<(), Error> {
        let mut scope = GenericScope::new();
        let arg_types = errors.block_exec(|errors| {
            if self.predicates.get(name.inner).is_some() {
                errors.push_error_message(name.pos, format!("predicate {} already defined", name.inner));
            }

            let arg_types = args.inner.iter()  
        ...
        let generics = scope.finish(errors).map_err(|_| format_err!("Type error"))?;

        self.predicates.insert(name.inner, Predicate {
            name: name.inner,
            arg_types,
            generics,
        });
    ...
    pub fn add_uninterpreted_function(&mut self, name: &Spanning<&'i str>, return_type: &TypeSpec<'i>, args: &Spanning<Vec<Spanning<TypeSpec<'i>>>>, errors: &mut ErrorContext<'i>) -> Result<(), Error> {
        let mut scope = GenericScope::new();
        let (arg_types, return_type) = errors.block_exec(|errors| {
            if self.functions.get(name.inner).is_some() {
                errors.push_error_message(name.pos, format!("function {} already defined", name.inner));
            }
            ...
            let arg_types = args.inner.iter()
                ...
                .collect::<Result<Vec<_>, _>>()
                .map_err(|_| format_err!("Type error"))?;
            let return_type = self.make_active(&mut scope, return_type, None, errors)
                .map_err(|_| format_err!("Type error"))?;
        ...
        self.functions.insert(name.inner, Function::Uninterpreted {
            name: name.inner,
            return_type,
            arg_types,
            generics
    ...
    pub fn add_numeric_function(&mut self, name: &Spanning<&'i str>, args: &Spanning<Vec<Spanning<TypeSpec<'i>>>>, errors: &mut ErrorContext<'i>) -> Result<(), Error> {
        let mut scope = GenericScope::new();
        ...
        self.functions.insert(name.inner, Function::Numeric {
            name: name.inner,
            arg_types,
            generics
        });
    ...
    fn make_active(&self, scope: &mut GenericScope<'i>, spec: &TypeSpec<'i>, parent: Option<TypeRef>, errors: &mut ErrorContext<'i>) -> Result<ActiveType, ()> {
        if let Some(type_ref) = self.get_type(spec.name.inner) {
            let args = transpose_option(spec.args.as_ref()
                .map(|args| {
                    let type_params = match &self.types[type_ref.0] {
                        Type::Uninterpreted { params, .. } => params,
                        _ => {
                            errors.push_error_message(args.pos, format!("type {} does not have any generic parameters", spec.name.inner));
                            return Err(())
                        }
                    };
                    if type_params.len() != args.len() {
                        errors.push_error_message(args.pos, format!("type {} expects {} generic parameter{}", spec.name.inner, type_params.len(), if type_params.len() == 1 { "" } else { "s" }));
                        return Err(());
                    }
                    args.iter()
                        .zip(type_params)
            ...
            Ok(ActiveType::Ref { to: type_ref, params: args.unwrap_or_else(Vec::new) })
        }
        else {
    ...
    pub fn instance_iter(&self, type_ref: TypeRef) -> Vec<GroundTerm<'i>> {
        let subinstances = self.subtypes.get(&type_ref)
            .map(|v| v.as_slice())
            .unwrap_or_else(|| &[])
            .iter()
        ...
        match &self.types[type_ref.0] {
            Type::Interpreted { instances, .. } =>
                subinstances.chain(
                    instances.iter().map(|n| GroundTerm::new_ident(n))).collect(),
            Type::Integer { min, max, .. } =>
                subinstances.chain((*min..=*max).map(GroundTerm::Number)).collect(),
...
impl<'i> GenericScope<'i> {
    ...
    fn get_generic_param(&mut self, name: Spanning<&'i str>, upper_bound: Option<TypeRef>) -> ActiveType {
        for (index, (existing_name, usages, current_upper_bound)) in self.generics.iter_mut().enumerate() {
            if existing_name.inner == name.inner {
                *usages += 1;
                if current_upper_bound.is_none() && upper_bound.is_some() { // FIXME find lowest upper bound
...
impl<'i> Function<'i> {
    ...
    pub fn arg_types(&self) -> &[ActiveType] {
        match self {
            Function::Numeric { arg_types, ..} |
            Function::Uninterpreted { arg_types, .. } => arg_types,
        }
    }

    pub fn generics(&self) -> &[(&'i str, Option<TypeRef>)] {
        match self {
            Function::Numeric { generics, .. } |
            Function::Uninterpreted { generics, .. } => generics,
        }
    }
