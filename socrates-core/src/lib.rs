...
#[cfg(test)] extern crate socrates_parser;
...
use socrates_ast::parsed::{DocumentItem, Definition};
...
) -> bool
{
    let result = match item.inner {
        DocumentItem::Definition(Spanning { inner: Definition::Type(spec), .. })
            => storage.add_uninterpreted_type(&spec, errors),
        DocumentItem::Definition(Spanning { inner: Definition::Instances(super_type, name, instances), .. })
            => storage.add_interpreted_type(&name, &super_type, &instances, errors),
        DocumentItem::Definition(Spanning { inner: Definition::IntegerType(name, min, max), .. })
            => storage.add_integer_type(&name, &min, &max, errors),
        DocumentItem::Definition(Spanning { inner: Definition::Predicate(name, args), .. })
            => storage.add_predicate(&name, &args, errors),
        DocumentItem::Definition(Spanning { inner: Definition::UninterpretedFunction(return_type, name, args), .. })
            => storage.add_uninterpreted_function(&name, &return_type, &args, errors),
        DocumentItem::Definition(Spanning { inner: Definition::NumericFunction(name, args), .. })
            => storage.add_numeric_function(&name, &args, errors),
        DocumentItem::Definition(Spanning { inner: Definition::CompletionBucket(name, _predicate), .. }) => {
            if buckets.get(name.inner).is_some() {
                errors.push_error_message(name.pos, format!("Bucket {} already exists", name.inner));
                return false;
            }
        ...
        DocumentItem::BucketedFormula(bucket, formula) => {
            match buckets.get(bucket.inner) {
                Some(_bucket) => (),
                None => {
                    errors.push_error_message(bucket.pos, format!("Bucket {} does not exist", bucket.inner));
                    return false;
                }
            }
...
fn emit_formula<'i, E: grounder::Emitter<'i>>(
    ...
    let formula = errors.block_exec(|errors|
        Ok(ident_resolver::resolve_identifiers(formula, storage, errors)))?;

    let formula = errors.block_exec(|errors|
        Ok(type_inferencer::type_check(formula, storage, errors)))?;
