...
use predicate_completion::Bucket;
use grounder::Emitter;
...
impl<'i> Context<'i> {
    ...
    pub fn exec_unit(&mut self, source: &'i str) -> Result<(), Error> {
        ...
        errors.block_exec(|errors| {
            ...
            for item in document {
                handle_item(&mut self.emitter, item, &mut self.types, errors, &mut self.buckets);
            }
    ...
    pub fn build_active_type(&self, name: &str, params: Vec<ActiveType>) -> ActiveType {
        ActiveType::Ref {
            to: self.types.get_type(name).unwrap_or_else(|| panic!("Could not find type {}", name)),
            params,
        }
    }
    ...
    pub fn assert_variable_type(
        ...
        assert!(inner_assert(&self.types, f, (s, v), ty), "Variable {}:{} not found", s, v);
    }
